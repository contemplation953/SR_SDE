...
% coeff=data.x;
coeff=load(sprintf('%s/global_params.mat',data_path)).x;
mhv1=data.mhv1;
shv1=data.shv1;
...
options = optimset('PlotFcns',@optimplotfval,'MaxFunEvals',1e9,'MaxIter',2e3,'TolFun ',1e-5);
%options = optimset('MaxFunEvals',1e9,'MaxIter',2e3,'TolFun ',1e-5);
