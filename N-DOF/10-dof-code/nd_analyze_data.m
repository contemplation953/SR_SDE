...
sh_p=3;
root_path=pwd();
data_path=sprintf('%s/N-DOF/10_dof_data',root_path);
...
for i=1:61
    ...
    data2=load(sprintf('%s/N-DOF/10_dof_data/H_%d.mat',root_path,i));
    p_data=data2.p_data;
...
save(sprintf('%s/10dofglobal_init_params.mat',data_path),'params_m','params_s','-v7.3');
save(sprintf('%s/10dofML_init_params.mat',data_path),'fd_val','fh_val','fmh_val','fsh_val','-v7.3');