...
for i=1:length(DD_list)
    data1=load(sprintf('%s/10_dof_data/H_%d.mat',data_path,i));
    pdf_data_matrix(i,:)=data1.p_data;
    
    data2=load(sprintf('%s/10_dof_res/H_%d.mat',data_path,i));
...
zlabel('pdf','FontSize',20);
saveas(gcf,sprintf('%s/10_dof_res/10dof_pdf_3d_data.png',data_path));
...
ylabel('D','FontSize',20);
zlabel('pdf','FontSize',20);
saveas(gcf,sprintf('%s/10_dof_res/10dof_pdf_3d_ml.png',data_path));
...
for i=1:length(VD_list)
    tic;
    data1=load(sprintf('D:/ChenXiYuan/workspace/SMl/sde/Ham-5-DOF/N-DOF/10_dof_data/H_%d.mat',i));
    pdf_data_matrix(i,:)=data1.p_data;
    
    data2=load(sprintf('D:/ChenXiYuan/workspace/SMl/sde/Ham-5-DOF/N-DOF/10_dof_res/H_%d.mat',i));
    pdf_mlres_matrix(i,:)=data2.pdf_ans;
end
...
hold off;
saveas(gcf,sprintf('%s/10_dof_res/10dof_pdf_demo.png',data_path));