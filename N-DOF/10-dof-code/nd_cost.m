function res=nd_cost(coeff,pm,ps,p_data,pst,m1_data,m2_data)
    coeff1=coeff(1:pm+1);
    coeff2=coeff(pm+2:end);

    [m_x,s_x]=funcFactory(coeff1,coeff2);
    md=moment_displace(pst,m_x,s_x,m1_data,m2_data,p_data);
...
function res=KL_Divergence(p_data,p_analytical,pst)
    p_data(p_data==0)=1e-10;
    p_analytical(p_analytical==0)=1e-10;
    res=trapz(pst,p_data.*log(p_data./p_analytical));
end

function res=moment_displace(pst,m_x,s_x,m1_data,m2_data,p_data)
    m1_ans=m_x(pst);
    m2_ans=(s_x(pst)).^2;
    d1=(m1_ans-m1_data);
    d2=(m2_ans-m2_data);
    res=sqrt((d1.*d1+d2.*d2)*p_data');
end
